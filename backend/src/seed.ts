...
async function bootstrap() {
    ...
    for (const item of teatinosData) {
        ...
        // Si el piso está vacío, extraerlo del nombre del departamento
        if (!item.piso || item.piso.trim() === '') {
            ...
            if (match) {
                ...
                // Extraer los primeros 1 o 2 dígitos como número de piso
        ...
        // B. Determinar tipología basándose en el nombre
        ...
        if (item.estado === 'VENTA') {
            ...
            estadoDb = 'Reservada'; // O 'Vendida' según lógica de negocio, pero 'OFERTA' suele ser Reserva.
        ...
        if (!unidad) {
            unidad = await unidadRepo.save({
                proyecto: proyecto,
                nombre: item.depto,
                tipologia: tipologia, // Usar la tipología determinada (STUDIO o DEPTO)
                valorUf: item.precio,
                piso: pisoNum,
                estado: estadoDb,
            ...
            // Actualizar TODOS los campos relevantes, incluyendo piso y tipología
        ...
        // D. Gestión de Cliente y Ficha (Solo si no está disponible)
        if (estadoDb === 'Reservada' || estadoDb === 'Vendida') {
            if (clientNameForFicha) {
                ...
                // Asumimos que si la unidad está reservada, debe tener una ficha activa
